...
int lsp_init(char *cmd[])
{
	int ifds[2], ofds[2];
	lsp_done();
	strcpy(lsp_root, "file://");
	getcwd(lsp_root + strlen(lsp_root), sizeof(lsp_root) - strlen(lsp_root));
	if (pipe(ifds) < 0)
		return 1;
	if (pipe(ofds) < 0) {
		fds_close(ifds);
...
static int lsp_open(char *path, char *ft)
{
	...
	if (!lsp_mtime) {
		...
		http_notify(1024 + strlen(body),
			...
			lsp_root, path, lsp_lang(ft), body);
		lsp_version = 0;
		...
		http_notify(1024 + strlen(body),
			...
			lsp_root, path, lsp_version, body);
		lsp_version++;
		lsp_mtime = mtime(path);
	}
	free(body);
